...
from dataset.camvid_aug import CamVidDataSet, CamVidValDataSet, CamVidTrainInform, CamVidTestDataSet
...
def build_dataset_test(dataset, num_workers, none_gt=False, f_scale=1):
    data_dir = os.path.join('./dataset/', dataset)
    dataset_list = dataset + '_trainval_list.txt'
    ...
    if dataset == "cityscapes":
        ...
        if none_gt:
            ...
            testLoader = data.DataLoader(
                CityscapesValDataSet(data_dir, test_data_list, mean=datas['mean'],f_scale=1), 
                batch_size=1, shuffle=False, num_workers=num_workers, pin_memory=True)
        ...
        testLoader = data.DataLoader(
            CamVidValDataSet(data_dir, test_data_list, mean=datas['mean'], f_scale=f_scale),
            batch_size=1, shuffle=False, num_workers=num_workers, pin_memory=True)
