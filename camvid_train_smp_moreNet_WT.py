...
def parse_args():
    ...
    # model and dataset
    parser.add_argument('--model', type=str, default='UNet', help="model name: (default ENet)")
    parser.add_argument('--mode', type=str, default=SAVE_FLAG[SEL_NUM], help="Mode selection: (default augumentation)")
    ...
    parser.add_argument('--lr', type=float, default=5e-4, help="initial learning rate") # 0.0005
...
def train_model(args, Seg_Model, Backbone):
    """
    args:
    ...
    if args.dataset == 'camvid':
        criteria = CrossEntropyLoss2d(weight=weight, ignore_index=ignore_index)
    elif args.dataset == 'camvid' and args.use_label_smoothing:
        criteria = CrossEntropyLoss2dLabelSmooth(weight=weight, ignore_index=ignore_index)

    elif args.dataset == 'cityscapes' and args.use_ohem:
        min_kept = int(args.batch_size // len(args.gpus) * h * w // 16)
        criteria = ProbOhemCrossEntropy2d(use_weight=True, ignore_index=ignore_index, thresh=0.7, min_kept=min_kept)
    elif args.dataset == 'cityscapes' and args.use_label_smoothing:
        criteria = CrossEntropyLoss2dLabelSmooth(weight=weight, ignore_index=ignore_index)
    elif args.dataset == 'cityscapes' and args.use_lovaszsoftmax:
        criteria = LovaszSoftmax(ignore_index=ignore_index)
    elif args.dataset == 'cityscapes' and args.use_focal:
        criteria = FocalLoss2d(weight=weight, ignore_index=ignore_index)
    else:
    ...
    args.savedir = (args.savedir + args.dataset + '_Four_HWD/' + args.model + 'bs'
                    + str(args.batch_size) + 'gpu' + str(args.gpu_nums) + 
                            "_" + str(args.train_type) + '_' + args.mode + '/')
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        # NO validation
        if (epoch+1) % 5000 == 0: #or epoch == (args.max_epochs - 1):
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        # save_predict(output, gt, img_name=name[0], dataset='camvid', save_path='./FastCNN_temp', #result/camvid
...
if __name__ == '__main__':
    ...
    ## smp.Unet, smp.DeepLabV3Plus, PAN
    # SMP_MODEL = [  smp.Unet, smp.DeepLabV3Plus,smp.Linknet, smp.FPN, smp.PSPNet, smp.UnetPlusPlus]
    SMP_MODEL = [smp.UnetPlusPlus, smp.DeepLabV3Plus, smp.PAN, smp.Linknet, smp.Unet, smp.FPN, smp.PSPNet, ] 
    # # SMP_MODEL = [smp.DeepLabV3,  smp.PAN, smp.Linknet, smp.DeepLabV3Plus] ## Set diconv Truesmp.Unet, smp.FPN,  smp.PSPNet, # smp.PSPNet, smp.Unet, smp.Linknet, smp.PAN, smp.DeepLabV3Plus,
                    # smp.Linknet,
    BACKBONE_NAME = [ "resnet18_wt", "resnet34_wt", "resnet50_wt",]#] # "resnet18_wt", "resnet34", "resnet50_wt",
    ...
    # BACKBONE_NAME = ["xception_wt", "xception"]#] # "resnet18_wt","resnet34_wt","resnet50_wt", "resnet101_wt", 'mobilenet_v2'
    ...
    for m in range(len(SMP_MODEL)):
        for n in range(len(BACKBONE_NAME)):
            ...
            print(args.model)
            train_model(args, Seg_Model=SMP_MODEL[m], Backbone=BACKBONE_NAME[n])
    ...
    # logFileLoc = os.path.join('/home/gpxu/vess_seg/vess_efficient/eval_model', 
    #                             args.dataset + '_' + args.model + '_trainingTime.txt')

    # # Save the result
    # logger = open(logFileLoc, 'a')
    # logger.write("Mean Time: %.2f s\n" % (training_time))    
