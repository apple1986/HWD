...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    with torch.no_grad():
        ...
        if all([i == j for i, j in zip(net_output.shape, gt.shape)]):
            ...
            y_onehot = label2onehot_v2(gt, num_classes=11)
...
def label2onehot_v2(target, num_classes=11):
    tensor_list = []
...
class SoftDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        dc = nominator / (denominator + 1e-8)  ## 分别计算每一类别的dc，然后平均，输出为每一类dice
...
class DC_and_CE_loss_V2(nn.Module):
    def __init__(self, weight=None, ignore_index=11, reduction="mean", aggregate="sum",
                            weight_ce=1, weight_dice=1,log_dice=False):
        """
        ...
        :param weight_dice:
        """
        super(DC_and_CE_loss_V2, self).__init__()
        ...
        self.ce = nn.CrossEntropyLoss(weight, ignore_index=ignore_index, reduction=reduction)
...
class AsymLoss(nn.Module):
    def __init__(self, apply_nonlin=softmax_helper, batch_dice=False, do_bg=True, smooth=1e-5,
                 square=False):
        """
