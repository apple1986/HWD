...
from networks import resnet_d as Resnet_Deep
from networks.wider_resnet import wider_resnet38_a2
from networks.deepv3 import _AtrousSpatialPyramidPoolingModule
from networks.nn.mynn import initialize_weights, Upsample
from networks.nn.operators import PSPModule
...
class DeepWV3PlusDecoupleEdgeBody(nn.Module):
    ...
    def __init__(self, num_classes, trunk='WideResnet38', criterion=None):
        ...
        self.edge_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(edge_dim), nn.ReLU(inplace=True))

        self.squeeze_body_edge = SqueezeBodyEdge(256, nn.BatchNorm2d)
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, trunk='seresnext-50', criterion=None, variant='D',
        ...
        self.ppm = PSPModule(2048, 256, norm_layer=nn.BatchNorm2d)
...
if __name__ == '__main__':
    ...
    img = torch.randn(3, 3, 256, 256).to(device)
    model = DeepR50PSP_m1_deeply(num_classes=19, criterion=None).to(device)
    out = model(img)
    print(out.shape)
    # print(model)