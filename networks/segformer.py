...
import sys
sys.path.append("./")
from .backbone import mit_b0, mit_b1, mit_b2, mit_b3, mit_b4, mit_b5, mit_b0_WT
...
class SegFormer_WT(nn.Module):
    def __init__(self, num_classes = 21, phi = 'b0', pretrained = False):
        super(SegFormer_WT, self).__init__()
        self.in_channels = {
            'b0': [32, 64, 160, 256], 'b1': [64, 128, 320, 512], 'b2': [64, 128, 320, 512],
            'b3': [64, 128, 320, 512], 'b4': [64, 128, 320, 512], 'b5': [64, 128, 320, 512],
        }[phi]
        self.backbone   = { 
            'b0': mit_b0_WT, 'b1': mit_b1, 'b2': mit_b2,
            'b3': mit_b3, 'b4': mit_b4, 'b5': mit_b5,
        }[phi](pretrained)
        self.embedding_dim   = {
            'b0': 256, 'b1': 256, 'b2': 768,
...
if __name__ == '__main__':
    ...
    model = SegFormer(num_classes=10)
    print(model)
    img = torch.randn(8, 1, 224, 224)
    pd = model(img)
    print(pd.shape)