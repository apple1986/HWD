...
# from builders.model_builder_nature import build_model
from builders.synapse_dataset_builder import build_dataset_train
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default="synapse", help="dataset: cityscapes or camvid")
    parser.add_argument('--input_size', type=str, default="224,224", help="input size of model")
...
def train_model(args, model):
    ...
    if args.dataset == 'synapse':
        ...
        ce_loss = CrossEntropyLoss2d(weight=weight)
        dice_loss = DiceLoss(args.classes)
        # criteria = ce_loss + dice_loss

    elif args.dataset == 'synapse' and args.use_label_smoothing:
    ...
    args.savedir = (args.savedir + args.dataset + '_SMP/' + args.model + '_bs'
                    + str(args.batch_size) + 'gpu' + str(args.gpu_nums) + "_" + str(args.train_type) + 'ce_dice' +'/')
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        lossTr, lr = train(args, trainLoader, model, ce_loss, dice_loss, optimizer, epoch)
        lossTr_list.append(lossTr)

        # validation  ##########notice I change the number
        if epoch % 5000 == 0 and epoch == (args.max_epochs - 1):
...
def train(args, train_loader, model, criterion_ce, criterion_dice, optimizer, epoch):
    """
    args:
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        loss_ce = criterion_ce(output, labels)
        loss_dice = criterion_dice(output, labels, softmax=True)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'cityscapes':
        ...
        args.classes = 9
        args.input_size = '224,224' #360,480'
        ignore_label = 11
    else:
        raise NotImplementedError(
            "This repository now supports two datasets: cityscapes and camvid, %s is not included" % args.dataset)
    ...
    # smp.UnetPlusPlus,
    SMP_MODEL = [smp.Unet,  smp.PAN]#,smp.DeepLabV3Plus,smp.PAN,]# smp.DeepLabV3,  smp.PAN, smp.Linknet,smp.Unet, smp.FPN, smp.PSPNet, smp.UnetPlusPlus, 
    ...
    # SMP_MODEL = [smp.Unet, smp.FPN]# smp.
    ...
    # BACKBONE_NAME = [
    #                  "resnet34_wt",   
    #                  "resnet50_wt",  ] # "resnet101_wt", "resnet18_wt", 
    ...
    if GPU_NUM == "0":
        ...
        # BACKBONE_NAME = ["resnet50_wt"]       #xception_wt   
        NUM_CHANNEL = 3   
    else:
        BACKBONE_NAME = ["resnet34",  
                      "resnet18", 
                     "resnet50", ] # "resnet101_wt",   
        # BACKBONE_NAME = ["resnet50"] #xception
        NUM_CHANNEL = 1
